...
(defun init-raymarch ()
  ...
  (unless *quad-3d*
    ...
    ;; 0 0 0 // TL
    (setf *quad-3d*
          (let* ((verts (make-gpu-array `((,(v! -1 -1 3) ,(v! 1 0 0) ,(v! 0 0))
...
(defvar *tmp2* (make-instance 'actor))
(defun draw-raymarching ()
  ...
  (with-fbo-bound (*ray-fbo*)
    ...
    (map-g #'raymarch-pipe *quad-3d*
           ...
           :view-world (m4:inverse (model->world *currentcamera*)))))
...
(defun-g calc-normal ((pos :vec3))
  (let* ((eps (v! .001 0))
         (nor (v! (- (distance-estimator (+ pos (s~ eps :xyy)) (v! 0 0 -30) 2f0)
                     (distance-estimator (- pos (s~ eps :xyy)) (v! 0 0 -30) 2f0))
                  (- (distance-estimator (+ pos (s~ eps :yxy)) (v! 0 0 -30) 2f0)
                     (distance-estimator (- pos (s~ eps :yxy)) (v! 0 0 -30) 2f0))
                  (- (distance-estimator (+ pos (s~ eps :yyx)) (v! 0 0 -30) 2f0)
                     (distance-estimator (- pos (s~ eps :yyx)) (v! 0 0 -30) 2f0)))))
    (normalize nor)))
  ...
  (let ((ret (v4! 0))
    ...
    (for (steps 0) (< steps max-steps) (++ steps)
         ...
         (let* ((p (+ from (* direction tt)))
                (d (distance-estimator p (v! 0 0 -10) 1f0)))
           (when (< d .001)
             (let* ((n (calc-normal p))
                    (light (dot (v! 0 1 0) n)))
               (setf ret (v4! (* light 1)))
...
(defun-g raymarch-vert ((vert g-pnt) ;;(pos :vec3)
  ...
  (let* ((pos   (pos vert))
         ...
         (ray (s~ (aref frustum-corners (int index))
                  :xyz))
